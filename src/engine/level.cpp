...
void level::level::update(float delta, int frame) {
  ...
  // graveyard destroyed here -- dead entities stay alive until render layers
  // are cleaned up
}
void level::level::render(int frame) {
  // draw the background
  DrawTextureRec(background_.get_texture(), view_frame_, Vector2{0.0f, 0.0f},
                 WHITE);
  ...
  auto render_precdicate = [this](entities::entity *&entity)
      -> bool { // auto is std::unique_ptr<entity>
...
void level::level::add_entity(std::unique_ptr<entities::entity> entity,
  ...
  auto dishwasher = dynamic_cast<entities::dishwasher *>(entity_raw);
  ...
  if (table != nullptr) {
    ...
    auto message = "[level::add_entity, queued table registration] "
                   ...
                   ", left_interaction_position: " +
                   vector_to_string(interaction_positions.left) +
                   ", right_interaction_position: " +
                   vector_to_string(interaction_positions.right);
    debug::log(message);
    std::unique_ptr<events::event> registered_table =
        std::make_unique<events::registered_table>(table);
  ...
  if (food_counter != nullptr) {
    ...
    debug::log("[level::add_entity, queued food counter registration] "
               "counter_id: " +
               std::to_string(entity_id) + ", position: " +
               vector_to_string(position) + ", interaction_position: " +
               vector_to_string(interaction_position));
    std::unique_ptr<events::event> registered_food_counter =
        std::make_unique<events::registered_food_counter>(food_counter);
    event_interface::queue_event(registered_food_counter);
  }
  if (waiter != nullptr) {
    debug::log("[level::add_entity, queued waiter registration] "
               "waiter_id: " +
               std::to_string(entity_id) +
               ", position: " + vector_to_string(position));
    std::unique_ptr<events::event> registered_waiter =
        std::make_unique<events::registered_waiter>(
            waiter, static_cast<size_t>(entity_id));
    event_interface::queue_event(registered_waiter);
  }
  if (dishwasher != nullptr) {
    debug::log("[level::add_entity, queued dishwasher registration] "
               "dishwasher_id: " +
               std::to_string(entity_id) +
               ", position: " + vector_to_string(position));
    std::unique_ptr<events::event> registered_dishwasher =
        std::make_unique<events::registered_dishwasher>(dishwasher);
    event_interface::queue_event(registered_dishwasher);
  }
}

void level::level::add_void_entity(std::unique_ptr<entities::entity> entity,
                                   size_t layer) {
  auto entity_id = entity->get_id();
  auto position = entity->get_position();
  next_entity_id_ = std::max(next_entity_id_, entity_id + 1);
  void_entities_.push_back(void_entity_record{std::move(entity), layer});
  debug::log("[level::add_void_entity, added entity to void] "
             "entity_id: " +
             std::to_string(entity_id) + ", layer: " + std::to_string(layer) +
             ", position: " + vector_to_string(position) +
...
void level::level::move_void_entity_toward_screen(entities::entity &entity,
                                                  int frame) {
  (void)frame;
  auto position = entity.get_position();
  const auto &box = entity.get_hitbox().get_box();
...
void level::level::insert_void_entity(void_entity_record record) {
  ...
  if (customer == nullptr) {
    debug::log("[level::insert_void_entity, inserted non customer entity] "
               "entity_id: " +
               std::to_string(entity_id) + ", layer: " + std::to_string(layer) +
               ", position: " + vector_to_string(position));
    return;
  ...
  debug::log("[level::insert_void_entity, inserted customer entity] "
             "entity_id: " +
             std::to_string(entity_id) + ", layer: " + std::to_string(layer) +
             ", position: " + vector_to_string(position));
...
int level::level::entity_id() { return next_entity_id_; }
...
void level::level::on_left_mouse_click_event(
    const events::left_mouse_click &event) {
  (void)event;
  return;
}
void level::level::on_move_view_frame_event(
...
void level::level::on_right_mouse_event(
  ...
  if (dog_id != -1) {
    ...
    auto dog_path =
        graph_.find_path(dog->get_position(), click_position, direction);
    dog_cast->set_path(dog_path);
  }
}
void level::level::on_build_customer_dog_event(
    const events::build_customer_dog &event) {
  auto position = event.get_position();
  auto dog_type = event.get_dog_type();
  auto dog_id = entity_id();
  auto new_dog = entities::e_builder.build_customer_dog(dog_id, dog_type,
                                                        position, std::nullopt);
  debug::log("[level::on_build_customer_dog_event, built customer dog] "
             "dog_id: " +
...
void level::level::on_send_dog_to_position_event(
    const events::send_dog_to_position &event) {
  ...
  std::unique_ptr<events::event> give_dog_path =
      std::make_unique<events::give_dog_path>(customer_id, path);
  event_interface::queue_event(give_dog_path);
}

void level::level::on_send_dog_to_station(
    const events::send_dog_to_station &event) {
  ...
  if (dog_record == id_entity_map_.end()) {
    debug::log("[level::on_send_dog_to_station, missing customer entity] "
               "customer_id: " +
               std::to_string(dog_id) +
  ...
  auto dog = static_cast<entities::dog *>(dog_record->second);
  auto position = (event.get_source() == std::nullopt)
                      ? dog->get_position()
                      : event.get_source().value();
  auto path = graph_.find_path(position, event.get_destination(),
                               dog->get_direction_scalar());

  dog->set_path(path, static_cast<int>(event.get_station_id()),
...
void level::level::on_dog_reached_station_event(
  ...
  auto dog_id = static_cast<int>(event.get_dog_id());
  debug::log("[level::on_dog_reached_station_event, dog entered station] "
             "dog_id: " +
             std::to_string(dog_id) +
...
void level::level::on_order_served_event(const events::order_served &event) {
  ...
  if (customer_record == id_entity_map_.end()) {
    debug::log("[level::on_order_served_event, missing customer entity] "
               "customer_id: " +
               std::to_string(customer_id) +
  ...
  auto customer =
      dynamic_cast<entities::customer_dog *>(customer_record->second);
  if (customer == nullptr) {
    debug::log("[level::on_order_served_event, entity is not customer dog] "
               "customer_id: " +
               std::to_string(customer_id) +
               ", order_id: " + std::to_string(event.get_order_id()) +
               ", table_id: " + std::to_string(event.get_table_id()));
  ...
  customer->set_eating(event.get_order_id(), event.get_table_id(),
...
std::unique_ptr<level::level> level::level_builder::build_main_level() {
  auto background = sprite::sprite(
      LoadTexture(entity_config::background_path),
      entity_config::background_attributes
          [entity_config::attributes::frame_width],
      entity_config::background_attributes
          [entity_config::attributes::frame_height],
      entity_config::background_attributes[entity_config::attributes::frames],
      entity_config::background_attributes
          [entity_config::attributes::animations]);

  auto view_frame = Rectangle{0.0f, 0.0f, static_cast<float>(GetScreenWidth()),
  ...
  // 0
  auto mack = entities::e_builder.build_mack(
      Vector2{level_config::edge_weight * 7, level_config::edge_weight * 4},
      main_level->entity_id());
  main_level->add_entity(std::move(mack), level_config::draw_layers::dogs);

  // 1
  auto khiri = entities::e_builder.build_khiri(
      Vector2{level_config::edge_weight * 4,
              static_cast<float>(level_config::edge_weight * 3.5)},
      main_level->entity_id());
  main_level->add_entity(std::move(khiri), level_config::draw_layers::dogs);

  // 2
  //  append the cursor
  auto cursor = entities::e_builder.build_cursor(GetMousePosition(),
                                                 main_level->entity_id());
  main_level->add_entity(std::move(cursor), level_config::draw_layers::cursor);

  auto first_table = entities::e_builder.build_table(
      Vector2{level_config::edge_weight * 6, level_config::edge_weight * 6},
      main_level->entity_id());
  main_level->add_entity(std::move(first_table),
                         level_config::draw_layers::stations);

  auto second_table = entities::e_builder.build_table(
      Vector2{level_config::edge_weight * 12, level_config::edge_weight * 12},
      main_level->entity_id());
  main_level->add_entity(std::move(second_table),
                         level_config::draw_layers::stations);

  auto food_counter = entities::e_builder.build_food_counter(
      Vector2{level_config::edge_weight * 18, level_config::edge_weight * 6},
      main_level->entity_id());
  ...
  if (auto *counter =
          dynamic_cast<entities::food_counter *>(food_counter.get())) {
    for (size_t i = 0; i < counter->max_capacity(); ++i) {
      counter->store(entities::e_builder.build_test_food(
          counter->get_position(), main_level->entity_id()));
    }
  }
